...
then
  ...
  echo ""
  echo "" 
  echo -e "Do you want to install XRAY(y/n)?"
  echo ""
  read ans
  if [ $ans == "y" ]; 
  then
      ...
      rm -rf /usr/local/etc/xray/config.json
      wget https://raw.githubusercontent.com/crixsz/XrayInstaller/configv2/config.json && mv config.json /usr/local/etc/xray/
      systemctl restart xray
      clear
      echo -e "${GREEN}Xray has been installed"
  else
